...
def run_eval(stereo_model, dataloader, scale, output_dir):
    ...
    for sample in pbar:
        ...
        for b in range(pred_disp.shape[0]):
            ...
            # ================================================================
            #  VISUALIZATION: [img0 | pred_disp | gt_disp | err_disp | pred_depth | gt_depth | err_depth]
            # ================================================================
...
def get_model_tag(args):
    if args.model_name == 'waft-stereo':
        return f'waft-stereo-{args.waft_variant}'
...
def main(args):
    ...
    vis_dir = output_dir / f'vis_scale{args.scale}_keyframes_{model_tag}'
    ...
    stereo_model = StereoDisparityEstimator(
        ...
        fs_ckpt_dir=args.fs_ckpt_dir,
        waft_variant=args.waft_variant,
        waft_repo_dir=args.waft_repo_dir,
        scale=args.scale,
        max_batch_size=1,
    )
    ...
    results["config"] = {
        "model_name": args.model_name,
        "waft_variant": args.waft_variant if args.model_name == 'waft-stereo' else None,
        "scale": args.scale,
    ...
    results_fname = output_dir / f'results_{model_tag}.json'
...
if __name__ == '__main__':
    ...
    parser.add_argument('--scale', type=float, default=1.0,
                        help='resolution scaling factor')

    # Model selection
    parser.add_argument('--model_name', default='foundationstereo',
                        choices=['foundationstereo', 'fast-foundationstereo', 'waft-stereo'],
                        help='Which stereo model to run.')
    parser.add_argument('--waft_variant', default='L', choices=['S', 'B', 'L'],
                        help='WAFT-Stereo size variant (only used if --model_name=waft-stereo).')

    # Repo / checkpoint paths
    parser.add_argument('--fs_ckpt_dir',
                        default='/home/jhan3/endo_stereo/FoundationStereo/pretrained_models/23-51-11',
                        help='(Fast-)FoundationStereo checkpoint directory.')
    parser.add_argument('--waft_repo_dir',
                        default='/home/jhan3/endo_stereo/WAFT-Stereo',
                        help='WAFT-Stereo repo root (expects configs/SynLarge/*.yaml and '
                             'ckpts/SynLarge/*.pth inside).')
